...
void sle_start_scan(void)
{
    ...
    osal_printk(
"[SLE_RSSI] start scan prepare\r\n"
);

    errcode_t ret = sle_set_seek_param(&param);
    osal_printk(
"[SLE_RSSI] sle_set_seek_param ret:0x%x\r\n"
, ret);

    ret = sle_start_seek();
    osal_printk(
"[SLE_RSSI] sle_start_seek ret:0x%x\r\n"
, ret);
}
...
{
    osal_printk(
"[SLE_RSSI] seek enable cbk status:0x%x\r\n"
, status);
}
...
{
    osal_printk(
"[SLE_RSSI] seek disable cbk status:0x%x\r\n"
, status);
}
...
void sle_sample_seek_result_info_cbk(sle_seek_result_info_t *seek_result_data)
{
    ...
    int8_t rssi = (int8_t)seek_result_data->rssi;
    oled_rssi_display_update(rssi);

    osal_printk(
"SLE_RSSI,ADDR=%02x:%02x:%02x:%02x:%02x:%02x,RSSI=%d\r\n",
    ...
    /* scan-only RSSI ranging stage: do not connect, keep scanning */
    return;
}
    ...
    errcode_t status)
{
    ...
    if ((g_recv_pkt_num % RSSI_AVG_COUNT) == 0) {
        sle_read_remote_device_rssi(conn_id); // 闂備焦妞垮鍧楀礉鐎ｎ剝濮虫い鎺戝€荤壕濂告煟閺冣偓濡炲潡宕ラ埀鐟€ssi闂備胶顫嬮崘鈺傛倷闂?
    }
    if (g_recv_pkt_num == 0) {
    ...
    errcode_t status)
{
    ...
    // server缂傚倷鐒﹀Λ蹇涘垂閺夊尅鑰挎い蹇撶墕缁犳娊鏌曟径娑㈡闁哄棭浜弻娑㈠箳閺冣偓閻ｇ禍ad_req婵犵數鍋為崹鐢告偋婵犲啫顕遍柛娑欐綑鐟欙箓骞栭幖顓炵仯缂佲偓婢跺⊕褰掑礂閼规澘顥濋梺鎼炲妼瀹曨剟顢氬▎鎰闁规鍣导鍥ㄧ箾閹寸偞鎯勫ù婊勭矎閵囨劙宕掗悙鑼槺闂佸憡渚楅崹鏉库枔閻樼數纾?
